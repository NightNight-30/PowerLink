...
入参来源: ods_credit_api_input_branch_company_df (每日由init SQL全量重建,已过滤分公司)
"""
...
from common.spark_utils import (get_spark, get_branch_company_list, has_success_today, write_api_records, get_supplementary_prepaid_companies, MAX_RETRY)
...
CUSTOMER_DT = None  # 指定分公司入参表分区日期，None=自动取MAX(dt)
...
print(f"分区dt: {dt}")
print(f"分公司入参表分区: {CUSTOMER_DT or '自动(MAX(dt))'}")
...
# HK/TW过滤(无论Phase1是否跳过,Phase2都需要用到,提前定义)
...
# 1001特殊: get_supplementary_prepaid_companies内部已做"预付款 ∩ 分公司入参表"过滤
...
if supp_companies and not INIT_MODE:
    print(f"\n{'=' * 60}")
    print(f"【补充跑批】新增预付款分公司 - 写入月度分区dt={last_batch_date}")
    print(f"{'=' * 60}")

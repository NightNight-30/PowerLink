...
from common.config_loader import load_config, get_interface_name, get_api_config, get_provider_config, should_run_today, is_prepaid_filter_enabled, get_monthly_day, is_hk_tw_filter_enabled, get_prepaid_run_months, get_run_dt, is_init_mode
...
print(f"预付款跑批月份: {get_prepaid_run_months(CONFIG, INTERFACE_KEY) or '每月跑批日(默认)'}")
...
if not should_run_today(CONFIG, INTERFACE_KEY, force_run=INIT_MODE):
    ...
    monthly_day = get_monthly_day(CONFIG)
    companies = get_company_list(spark, frequency=frequency, monthly_day=monthly_day, customer_dt=CUSTOMER_DT, force_all=INIT_MODE, exclude_hk_tw=exclude_hk_tw, prepaid_run_months=prepaid_run_months)
    if not companies:

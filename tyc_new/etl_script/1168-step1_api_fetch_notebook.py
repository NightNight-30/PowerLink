# -*- coding: utf-8 -*-
"""【Notebook版】天眼查1168接口 - API数据拉取
核心改动: 查询即计费接口，失败不重试，直接返回失败记录
前置条件: Cell1已执行notebook_init
"""

from common.config_loader import load_config, get_interface_name, get_api_config, should_run_today, is_prepaid_filter_enabled, get_monthly_day
...
def call_api(keyword):
    ...
    try:
        ...
        if error_code == 0:
            ...
            return ('FAILED', (error_code, api_result))
...
def process_company(keyword):
    ...
    status, result = call_api(keyword)

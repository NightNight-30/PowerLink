...
# -*- coding: utf-8 -*-
"""
【Step2】天眼查819接口 - 数据解析(Databricks版)
...
解析规则（与原版相同）：
  ...
  Number时间戳        → datetime字符串（≥1e10为毫秒）
  简单字段            → 驼峰→下划线命名 + 显式映射

入库方式：动态分区覆盖（全量刷新dt分区）

执行方式：
  spark-submit 819-step2_data_parse.py [公司名]
"""
...
def parse_819_data(api_result: Dict, keyword: str, record_id: int) -> Dict:
    """解析819接口数据，返回一行记录"""

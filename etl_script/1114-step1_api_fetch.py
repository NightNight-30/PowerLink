...
# -*- coding: utf-8 -*-
"""
【Step1】天眼查1114接口 - API数据拉取（含翻页+重试机制）

功能：
  1. 从customer_info表获取公司列表
  2. 调用1114API（支持翻页），合并所有页数据存入api_call_record表
  3. 幂等检查：当天已有成功记录则跳过
...
⚠️ 特别备注：
  ...
  保守方案：若未来数据量超出JSON存储上限(约1GB)，需考虑分页存储或LONGTEXT替代

执行方式：
  python3 1114-step1_api_fetch.py [公司名]
"""
